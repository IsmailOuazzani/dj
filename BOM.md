...
Each subsystem lists the chosen part, supplier, approximate cost, rationale, and product URL. "TBD" means we haven't picked yet (see open decisions at the bottom). Per CLAUDE.md, every part row carries its Digikey CA product URL in a dedicated `Link` column — bare text, triple-click-copyable.

---

## MCU

| Part | Qty | Source | Cost | Rationale | Link |
|---|---|---|---|---|---|
| Arduino Nano RP2040 Connect | 1 | On hand | — | Native USB-MIDI capable, dual-core Cortex-M0+ @ 133 MHz with PIO for jog quadrature decoding + WS2812 driving without burning CPU. | https://www.digikey.ca/en/products/result?keywords=Arduino+Nano+RP2040+Connect |
...
| Part | Qty | Source | Cost | Rationale | Link |
|---|---|---|---|---|---|
| SparkFun CD74HC4067 16-channel analog mux breakout (BOB-09056) | 1 | Digikey `1568-09056-ND` | $9.75 | One chip covers all 17 analog signals (16 via mux + 1 direct ADC for the 17th). Breadboard-friendly headers. CD4067 DIP is EOL, this is the cleanest breadboard-compatible path. | https://www.digikey.ca/en/products/result?keywords=1568-09056-ND |
| **Subtotal** | | | **~$9.75** | | |

## Pots (the actual pots and faders)

| Subsystem | Qty | Pick | Notes | Link |
|---|---|---|---|---|
...
| Subsystem | Qty | Pick | Digikey P/N | Cost (USD)* | Notes | Link |
|---|---|---|---|---|---|---|
| Pitch faders | 2 | Bourns **PTL01-15R0-103B1** (100 mm, 10 kΩ, B-taper linear) | `PTL01-15R0-103B1-ND` | $9.15 ea / $18.30 | Full 100 mm DJ-fader length, on Digikey. Pro-audio PTL series body. | https://www.digikey.ca/en/products/detail/bourns-inc/PTL01-15R0-103B1/6605956 |
| Channel faders | 2 | Bourns **PTL45-15O0-103B2** (45 mm, 10 kΩ, linear B-taper) | `118-PTL45-15O0-103B2-ND` | $3.86 ea / $7.72 | Consolidated to the same SKU as the crossfader — one fader part across the deck strip. The previous `103A3` pick was both unavailable on Digikey CA and audio-taper (not linear, per the Bourns PTL datasheet "A vs B" code). Linear is correct for DJ channel volume; Mixxx applies its own fader curve. | https://www.digikey.ca/en/products/detail/bourns-inc/PTL45-15O0-103B2/16356348 |
| Crossfader | 1 | Bourns **PTL45-15O0-103B2** (45 mm, 10 kΩ, linear) | `118-PTL45-15O0-103B2-ND` | $3.86 | Best Digikey-orderable crossfader. Carbon-element wiper — will scratch eventually; route panel cutout to standard 45 mm Mini-Innofader footprint as a v2 upgrade path. Unit price reconciled with the channel-fader row (same SKU). | https://www.digikey.ca/en/products/detail/bourns-inc/PTL45-15O0-103B2/16356348 |
...
| Jog wheels | 2 | Adafruit **AS5600 STEMMA QT breakout** (PID 6357) | `1528-6357-ND` | $8.93 ea / $17.86 | Magnetic angle sensor, 4096 steps/rev, I²C. Contactless = no detent, unlimited life. **AS5600 has a hard-fixed I²C address (`0x36`) with no straps** — the two chips cannot share one bus. Split across the RP2040's two I²C peripherals: deck-A AS5600 on primary `Wire` (SDA0/SCL0, shared with MCP23017s + NeoKeys); deck-B AS5600 on secondary `Wire1` (SDA1/SCL1) on two free GPIOs. Adds 2× extra 4.7 kΩ pull-ups for the second bus (see Buttons section) and zero new ICs. Requires the matching diametric magnet (next row) glued axially to the jog spindle; mechanical bearing for the platter is on the builder. Breadboard-friendly (headers ship un-soldered). | https://www.digikey.ca/en/products/detail/adafruit-industries-llc/6357/26832926 |
...
| Adafruit relegendable MX keycaps, 10-pack (PID 5039) | 1 | `1528-5039-ND` | ~$7.92 | $7.92 | https://www.digikey.ca/en/products/result?keywords=1528-5039-ND |
| **Subtotal** | | | | **~$60.78 CAD** | |
...
| Analog mux (SparkFun BOB-09056 CD74HC4067 breakout) | $9.75 |
| Pitch faders (Bourns PTL01 ×2, 100 mm) | $18.30 |
| Crossfader (Bourns PTL45-15O0-103B2, 10 kΩ) | $3.86 |
| Channel faders (Bourns PTL45-15O0-103B2 ×2, 10 kΩ) | $7.72 |
| Jog encoders (Adafruit AS5600 ×2) | $17.86 |
...
| Hotcue keys (NeoKey 1×4 ×2 + MX switches ×8 + keycaps 10-pk) | $60.78 |
| Knob caps (Cliff CL170822BR ×16) | $20.00 |
| Fader caps (Davies 1300-B ×5) | $10.10 |
| Tactile button caps (E-Switch 1RBLK ×19) | $5.89 |
| **Pre-tax pre-shipping subtotal** | **~$244.20 CAD** |
| Shipping (digikey.ca, free over $100 CAD) | $0 |
| Tax (ON HST 13 % example; varies by province) | ~$31.75 |
| **Estimated delivered total (ON)** | **~$275.95 CAD** |
| Budget cap | $300.00 |
| **Headroom** | **~$24.05 CAD under** |

All v1 component decisions are locked, including the SPI/STEMMA swap for the I/O expanders forced by the new in-stock-now rule. Prices reconciled against Digikey CA qty-1 as of 2026-07-05. Pre-tax sits ~$56 under the $300 CAD spec cap; delivered (with ON HST) sits ~$24 under, leaving margin for shipping variance, qty-tier price drift, or one impulse add at order time.
